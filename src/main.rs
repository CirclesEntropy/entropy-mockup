...
use crate::simulator::*;
use crate::resource::{Resource, ResourceKind, LogicHash, Denomination, quantity_of_denomination };
...
fn main() {
    ...
    let ptxp = txs.iter().map(|x| simulator::is_valid_ptx(&logics, x)).collect::<Vec<_>>();
    println!("{:?}", ptxp);
...
fn init_logicfns() -> HashMap<LogicHash, LogicFunction> {
    ...
    fns.insert(1 as u8, alice_logic);
    ...
    pub fn alice_intent_logic (kind: ResourceKind, tx: PartialTx) -> bool {
        ...
        kind == ResourceKind::Created ||
            quantity_of_denomination(dolphin, &created) == 1 && (quantity_of_denomination(octopus, &created) == 1 ||  quantity_of_denomination(starfish, &created) == 2)
    }
...
mod alice {
    use super::*;
    use crate::resource::{LogicHash, ResourceKind, quantity_of_denomination};
    use crate::partialtx::{LogicFunction, PartialTx};

    pub fn ptx () -> PartialTx {
        PartialTx {resources: vec!((ResourceKind::Created, alice_intent().set_value(1)), (ResourceKind::Consumed, octopus().set_value(1)), (ResourceKind::Consumed, starfish().set_value(1)))
        }
    }
}
...
mod bob {
    ...
    pub fn ptx () -> PartialTx {
         PartialTx {
            resources: vec!((ResourceKind::Created, octopus().set_value(1)), (ResourceKind::Consumed, dolphin().set_value(1)))
        }
    }
}
...
mod solver {
    ...
    pub fn ptx () -> PartialTx {
        PartialTx {resources: vec!((ResourceKind::Consumed, alice_intent().set_value(1)), (ResourceKind::Created, dolphin().set_value(1)), (ResourceKind::Created, starfish().set_value(2)))
        }   
    }
}
